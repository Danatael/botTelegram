...
export async function GET() {
  try {
    ...
    const entradas = await prisma.entradas.findMany({
      where: {
        hora_entrada: { gte: today, lt: tomorrow },
      },
      include: {
        empleado: true,
    ...
    rows.forEach((row, rowIdx) => {
      ...
      // Fondo zebra
      if (rowIdx % 2 === 0) {
        page.drawRectangle({ x, y: y - rowHeight, width: totalTableWidth, height: rowHeight, color: rgb(0.98, 0.98, 0.98) });
      }
      ...
      if (maxLines > 1) {
        if (rowIdx % 2 === 0) {
          page.drawRectangle({ x: startX, y: y - actualRowHeight, width: totalTableWidth, height: actualRowHeight, color: rgb(0.98, 0.98, 0.98) });
        }
        let colX2 = startX;
        for (let i = 0; i <= headers.length; i++) {
      ...
      page.drawLine({ start: { x: startX, y: y - actualRowHeight }, end: { x: startX + totalTableWidth, y: y - actualRowHeight }, thickness: 1, color: rgb(0.8,0.8,0.8) });
      y -= actualRowHeight;
    ...
    page.drawLine({ start: { x: startX, y: startY }, end: { x: startX + totalTableWidth, y: startY }, thickness: 1, color: rgb(0.5,0.5,0.5) });
  ...
  // Utilidad para dividir texto en varias líneas según ancho máximo (por palabras)
  ...
  // Utilidad para dividir texto en varias líneas según ancho máximo (por caracteres, sin partir palabras)
  function splitTextByWidth(text: string, maxWidth: number, font: any, fontSize: number) {
    let lines: string[] = [];
    let currentLine = '';
